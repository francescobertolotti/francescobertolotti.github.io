...
function applyRuntimeModeUI() {
  ...
  if (getActiveApiKey()) {
    setStaticModeStatus("OpenAI key is configured for static mode.", "ok");
  } else {
...
async function normalizeToEnglish({ apiKey, model, draftText }) {
  const response = await callOpenAIResponses({
    ...
    input: [
      ...
      {
        role: "user",
        content: [{ type: "input_text", text: draftText }]
      }
    ]
  });
...
function downloadLatestExam() {
  if (runtimeMode === "backend") {
    ...
    window.location.href = `/api/generated-pdfs/${encodeURIComponent(latestPdfFileName)}`;
    return;
  }
...
function downloadLatestExcel() {
  if (runtimeMode === "backend") {
    ...
    window.location.href = `/api/generated-excels/${encodeURIComponent(latestExcelFileName)}`;
    return;
  }
